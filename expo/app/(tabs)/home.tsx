...
export default function HomeScreen() {
  const { user, getTodayCheckin, getTodayPlan, getStreak, getRecentCheckins, getNutritionProgress } = useUserStore();
  ...
  useEffect(() => {
    if (user === null || (user && !user.onboardingComplete)) {
  ...
  const getEnergyTrend = () => {
    ...
    const recent = energyLevels.slice(0, 3).reduce((a, b) => a + b, 0) / 3;
    const older = energyLevels.slice(3).reduce((a, b) => a + b, 0) / (energyLevels.length - 3);
